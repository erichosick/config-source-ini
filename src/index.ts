...
/**
 * IniSource supports loading configuration and settings from a file.
 * Supported file types are: .json, .ts.
 * NOTE: .yml and others will be moved to their own libraries.
 */
export default class IniSource implements IConfigSource {
  #fileAbsolutePath: string;

  #rootOffset: string;

  /**
   * Creates in instance of a IniSource.
  ...
  public async loadConfig(): Promise<ISourceType> {
    return new Promise<ISourceType>((resolve, reject) => {
      let fileExtension;
      try {
        fileExtension = IniSource.fileExtension(this.#fileAbsolutePath);
      } catch (exception) {
        reject(exception);
      }
